	...
	, m_deferred(m_recorder)
{
	if (!m_source)
	{
		throw std::invalid_argument(
			"caching_memory_allocator::caching_memory_allocator: A memory "
			"source is required."
		);
	}
}
...
)
{
	return allocate(
		size,
		alignment,
		queue_hint ? queue_handle(command_queue::cast(*queue_hint))
		           : queue_handle()
	);
}
...
)
{
	...
	{
		throw std::invalid_argument(
			"caching_memory_allocator::allocate: The alignment must be a "
			"power of two."
		);
	}
	...
	{
		throw std::invalid_argument(
			"caching_memory_allocator::allocate: The alignment exceeds the "
	...
	// somewhere it can no longer be handed out from. An empty buffer would
	// have no address of its own, so it gets the smallest block there is.
	...
	auto &block = acquire_block(size, queue);
	try
	{
		// The source is what took the memory, so it is what knows whether the
		// host can reach it. Its own pointer rather than the device one, since
		// the two only happen to coincide where the addresses are unified.
		return std::make_shared<buffer>(
			shared_from_this(),
			block,
			m_source->is_host_accessible() ? block.get_data() : nullptr
		);
	}
	catch (...)
	{
...
void caching_memory_allocator::recycle(
	memory_block &block,
...
memory_block* caching_memory_allocator::take_over_foreign_block(
	std::size_t size,
	const queue_handle &queue
)
{
...
memory_block& caching_memory_allocator::grow(std::size_t size)
{
...
std::size_t
caching_memory_allocator::get_heap_size(std::size_t size) const noexcept
{
...
std::size_t caching_memory_allocator::trim_locked()
{
