...
{
	// Ordered by queue first and size second, so the best fit is the first
	// block that is large enough and belongs to the requested queue.
...
bool memory_block_pool::is_partition(const memory_block &block) const noexcept
{
	...
	const auto next = std::next(ite);
	return next != m_blocks.cend() && next->get_heap() == heap;
}
